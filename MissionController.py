...
class MissionController:
    ...
    def _handle_takeoff(self):
        """
        TAKEOFF: Kalkış ve seyir yüksekliğine çıkma.
        """
        logger.info("=" * 50)
        logger.info("DURUM: TAKEOFF - Kalkış")
        logger.info("=" * 50)

        # Arama paterni oluştur
        search_waypoints = self.navigation.generate_search_pattern(
            center_lat=config.SEARCH_AREA_CENTER_LAT,
            center_lon=config.SEARCH_AREA_CENTER_LON,
            length=config.SEARCH_AREA_LENGTH,
            width=config.SEARCH_AREA_WIDTH,
            spacing=60,  # metre
        )

        # Arama misyonunu yükle
        if not self.navigation.create_search_mission(search_waypoints):
            logger.error("Arama misyonu yüklenemedi!")
            self._transition_to(MissionState.ERROR)
            return

        # ARM ve AUTO mod
        if not self.navigation.arm():
            logger.error("İHA arm edilemedi!")
            self._transition_to(MissionState.ERROR)
            return

        if not self.navigation.set_mode("AUTO"):
            logger.error("AUTO moda geçilemedi!")
            self._transition_to(MissionState.ERROR)
            return

        logger.info(f"Kalkış başlatıldı, hedef irtifa: {config.SEARCH_ALTITUDE}m")

        # İrtifaya ulaşmayı bekle
        target_alt = config.SEARCH_ALTITUDE * 0.90  # %90 yeterli

        for _ in range(120):  # 60 saniye bekle
            loc = self.navigation.get_location()
            if loc and loc[2] >= target_alt:
                logger.info(f"✓ Hedef irtifaya ulaşıldı: {loc[2]:.1f}m")
                self._transition_to(MissionState.SEARCH)
                return
            time.sleep(0.5)

        logger.warning("Kalkış zaman aşımı, aramaya devam ediliyor...")
        self._transition_to(MissionState.SEARCH)
